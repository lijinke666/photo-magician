    ...
    ; ((name, definition) => {
        ...
        class ImageMagician {
            ...
            cover,
                ...
                if (!isTextMode && !isImageMode) throw new Error('mode it must be "text" of "image" ')
                return new Promise((res, rej) => {
                    const ext = this.getCoverExt(cover)
                    const [sx, sy] = coordinate
                    this.createImageNode(cover).then(async (img) => {
                        this.waterMarkCanvas = document.createElement('canvas')
                        this.waterMarkCtx = this.waterMarkCanvas.getContext('2d')

                        //绘制图片水印
                        if (isImageMode) {
                            const waterMarkImg = await this.createImageNode(waterMark, img.width, img.height)
                        ...
                        this.createImageNode(waterMarkData).then((waterMarkNode) => {
                            res(waterMarkNode)
                        }).catch(rej)
                    }).catch(rej)
            ...
            */
            ...
            cover,
                ...
                return new Promise((res, rej) => {
                    ...
                    this.createImageNode(cover, clipWidth, clipHeight).then((img) => {
                        ...
                        this.createImageNode(clipImageData).then((clipImageNode) => {
                            res(clipImageNode)
                        }).catch(rej)
                    }).catch(rej)
                })
            ...
            transFormImageData(filterType, suffix) {
                ...
                let imageData = this.ctx.getImageData(0, 0, this.canvas.width, this.canvas.height) //像素信息
                    ...
                    case this.imageFilterConfig['relief']:
                        ...
                        for (let x = 1; x < copyImageData.width - 1; x++) {
                            for (let y = 1; y < copyImageData.height - 1; y++) {
                                ...
                                let nr = copyImageData.data[aidx + 0] - copyImageData.data[bidx + 0] + 128
                                let ng = copyImageData.data[aidx + 1] - copyImageData.data[bidx + 1] + 128
                                let nb = copyImageData.data[aidx + 2] - copyImageData.data[bidx + 2] + 128
            ...
            cover,
                ...
                return new Promise((res, rej) => {
                    ...
                    this.createImageNode(cover).then((img) => {
                        ...
                        this.createImageNode(filterData).then((filterImageNode) => {
                            res(filterImageNode)
                        }).catch(rej)
            ...
            cover,
                ...
                return new Promise((res, rej) => {
                    this.createImageNode(cover).then((img) => {
                        this.ctx.drawImage(img, 0, 0, img.width, img.height)
                        const compressImage = this.canvas.toDataURL('image/jpeg', Number(quality))
